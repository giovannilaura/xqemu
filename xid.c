...
void checkButton(int pbutton,int pressed ) {
int button;
button=pbutton;
if (pbutton > 6) {
        ...
        int mask = (1 << (button-GAMEPAD_DPAD_UP));
        if (pressed) {
        ...
        printf("pbutton = %d, button=%d WBUTTONS %d\n" ,pbutton,button,s->in_state.wButtons);
        return;
}
if (!pressed) {
  s->in_state.bAnalogButtons[button]= 0x0;
}
else 
  s->in_state.bAnalogButtons[button]= 0xff;
}

void checkDpad (int button,int pressed) {
       int mask;
       mask = (1 << (button));
        s->in_state.wButtons &= ~mask;
        if (pressed) {
...
void checkTrigger(int trig,int pressed) {
int button;
...
//usato per assi 3 e 4
int dead_or_value2(int value) {
    if ((value > DEAD_ZONE && value > 0) || (value < -DEAD_ZONE && value < 0)) {
        //FIXME("VALUE=\n",value);
        return value;
    }
...
void doIt() 
{
	...
	// timestamp packet
            ...
            if (e.type == JS_EVENT_AXIS) {
                int value = e.value;
                if (e.number == 0) {
                   if (dead_or_value2(e.value) > 0) {
                      s->in_state.sThumbLX=value;
                   }

                   else if (dead_or_value2(e.value) < 0) {
                      s->in_state.sThumbLX=value;
                   }
                   else {
                     s->in_state.sThumbLX=0;
                   }
               ...
               int request, int value, int index, int length, uint8_t *data)
{
    ...
    if (ret >= 0) {
        printf("xid handled by usb_desc_handle_control: %d\n", ret);
        return;
    }
    ...
    case ClassInterfaceRequest | HID_GET_REPORT:
        printf("xid GET_REPORT 0x%x %d \n", value,length);
        if (value == 0x100) { /* input */
            assert(s->in_state.bLength <= length);
            s->in_state.bReportId++; /* FIXME: I'm not sure if bReportId is just a counter, JoeFix probably it is just a counter */
            //s->in_state.bReportId=getTickCount();
    ...
    case VendorInterfaceRequest | USB_REQ_GET_DESCRIPTOR:
        printf("xid GET_DESCRIPTOR 0x%x\n", value);
        if (value == 0x4200) {
    ...
    case VendorInterfaceRequest | XID_GET_CAPABILITIES:
        printf("xid XID_GET_CAPABILITIES 0x%x\n", value);
        /* FIXME: ! */
    ...
    case ((USB_DIR_IN|USB_TYPE_CLASS|USB_RECIP_DEVICE)<<8)
        ...
        /* FIXME: ! */
        printf("xid unknown xpad request 0x%x: value = 0x%x\n",
                request, value);
...
static void usb_xid_handle_data(USBDevice *dev, USBPacket *p)
{
    ...
    case USB_TOKEN_IN:
        ...
        if (p->ep->nr == 2) {
            ...
            if (s->in_dirty) { //JoeFix removed at forst but then i think is better with
...
{   int i;
    USBXIDState *st = DO_UPCAST(USBXIDState, dev, dev);
    usb_desc_init(dev);
    fdJoy=open("/dev/input/js0",O_RDONLY | O_NONBLOCK);  

    st->intr = usb_ep_get(dev, USB_TOKEN_IN, 2);

    st->in_state.bLength = sizeof(st->in_state);
    st->out_state.length = sizeof(st->out_state);
    //st->kbd_entry = qemu_add_kbd_event_handler(xbox_gamepad_keyboard_event, st);
    st->xid_desc = &desc_xid_xbox_gamepad;
    s=st;
    s->in_dirty=true;
    s->in_state.wButtons=0x0;
    for (i=0;i < 7; i++) {
        s->in_state.bAnalogButtons[i]=0x0;
    }
    return 0;
}
