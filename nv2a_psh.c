...
int doCastToVec3=0; //JoeFix implicit cast
    ...
    case PS_REGISTER_DISCARD:
        if (is_dest) {
            return qstring_from_str("");
             ...
             if (doCastToVec3) {
                  return qstring_from_str("vec3(0.0)");
             }
...
static QString* psh_convert(struct PixelShader *ps)
{
    ...
    for (i = 0; i < 4; i++) {
        ...
        case PS_TEXTUREMODES_DOT_RFLCT_SPEC:
            printf("DOT REFLECT SPEC EXPECTED\n");
             //qstring_append_fmt(vars, "vec4 t%d = vec4(0.0); /* PS_TEXTUREMODES_NONE */\n",i);
            //JoeFix DOT NOT SPECULAR 
            qstring_append_fmt(vars, "vec4 t%d = vec4(dot(pT%d.xyz, t%d.rgb));\n",i, i, ps->input_tex[i]);
             //JoeFix Halo
            //assert(false);
            break;
        case PS_TEXTUREMODES_DOT_RFLCT_DIFF:
              printf("DOT REFLECT DIFF EXPECTED\n");
          
            //JoeFix DOT NOT SPECULAR Halo
            qstring_append_fmt(vars, "vec4 t%d = vec4(dot(pT%d.xyz, t%d.rgb));\n",i, i, ps->input_tex[i]);
            break;    
        case PS_TEXTUREMODES_DOT_ZW:
              printf("DOT ZW EXPECTED\n");
          
            //JoeFix Guess HALO
                        qstring_append_fmt(vars, "vec4 t%d = vec4(dot(pT%d.xyzw, t%d.rgba));\n",i, i, ps->input_tex[i]);
        ...
        case PS_TEXTUREMODES_BUMPENVMAP:
            if (!ps->state.rect_tex[i]){ //JoeFix GUN VALKYRIE
                 sampler_type = "sampler2D";
            }
            else {
                 sampler_type = "sampler2DRect";
        ...
        case PS_TEXTUREMODES_DOTPRODUCT:
            qstring_append_fmt(vars, "vec4 t%d = vec4(dot(pT%d.xyz, t%d.rgb));\n",
                               i, i, ps->input_tex[i]);
            break;
        case PS_TEXTUREMODES_DOT_ST: //JoeFix
               printf("DOT ST EXPECTED\n");
          
            qstring_append_fmt(vars, "vec4 t%d = vec4(dot(pT%d.xyz, t%d.rgb));\n",
                               i, i, ps->input_tex[i]);
            printf("JoeFix DOUBLE STEAL WHAT IS DOT_ST?\n");
            break;
        default:
            printf("SHOULD NOT BE HERE\n");
            printf("%x\n", ps->tex_modes[i]);                   

            assert(false); //JoeFix HAlo //texture NONE
            qstring_append_fmt(vars, "vec4 t%d = vec4(0.0); /* PS_TEXTUREMODES_NONE */\n",i);
            break;
        }
